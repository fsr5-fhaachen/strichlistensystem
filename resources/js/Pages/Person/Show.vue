<template>
    <LayoutContainer>
        ...
        >
            Du wirst in
            <template v-if="redirectCountdown == 1"> einer Sekunde </template>
            <template v-else> {{ redirectCountdown }} Sekunden </template>
            auf die Startseite geleitet.
        </p>
        <div class="grid gap-y-4 md:gap-x-4 col-span-4 md:grid-cols-3">
            <PersonCard :canBeHovered="false" :person="person" />
        ...
        >
            ...
            <ul v-if="articleActionLogs" class="flex flex-col gap-3">
                <li
                    v-for="articleActionLog in articleActionLogs"
                    :key="articleActionLog.id"
                    class="flex flex-col lg:flex-row gap-2 items-center"
                >
                    <span
                        class="text-gray-600 dark:text-gray-400 font-bold lg:w-60"
                        >{{ articleActionLog.createdAtFormatted }}</span
                    >
                    <span
                        v-if="articleActionLog.deleted_at"
                        class="text-lg text-red-800 dark:text-red-500"
                    >
                        Du hast den Artikel "<span class="font-bold">{{
                    ...
                    <a
                        v-if="
                            !articleActionLog.deleted_at &&
                            articleActionLog.cancelUntilTimestamp * 1000 >=
                                Date.now()
                        "
                        class="text-red-700 dark:text-red-400 uppercase font-bold text-lg cursor-pointer"
                        @click="cancel(articleActionLog)"
                        >[stornieren]</a
                    >
                </li>
            </ul>
    ...
    <div
        ...
        v-if="openQRCodeModal && authLink"
    >
        <div class="flex min-h-screen items-center justify-center">
            <div
                class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity justify-center"
                @click="openQRCodeModal = false"
            ></div>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const buy = (article, amount) => {
            ...
            router.post(
                "/person/" + props.person.id + "/buy/" + article.id,
                {
                    amount,
                },
                {
        ...
        const cancel = (articleActionLog) => {
            redirectCountdown.value = 30;
            router.post(
                "/person/" + props.person.id + "/cancel/" + articleActionLog.id,
                null,
                {
                    preserveScroll: true,
                }
        ...
        const generateAuthToken = () => {
            ...
            axios
                .post("/person/" + props.person.id + "/generate-auth-link")
                .then((res) => {
